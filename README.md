...
当前学习入口统一为 [秋招版本](docs/秋招版本.md)：先解释已投递简历对应的 Blackboard、分层记忆与 Skill 主线，最后讲后续新增功能。旧学习稿和审查记录在本地 `docs/archive/2026-09-19/` 归档，不纳入远端仓库。

2026-09-19 已增量迁入学生历史会话，以及独立的回答质量、风险分类、单／多 Agent 对比评测；原 Agent Runtime、安全、记忆和工具治理实现保留。历史会话入口为 `GET /api/chat/sessions` 与 `GET /api/chat/sessions/{session_id}`，只向当前学生返回自己的会话。评测用法、数据口径与验证范围见《秋招版本》末尾；Mock 验收结果不代表真实模型效果。
