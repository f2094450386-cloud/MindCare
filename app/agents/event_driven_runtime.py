...
from app.services.memory import RedisShortTermMemoryStore, load_session_history


class EventDrivenAgentRuntimeService(AgentRuntimeService):
    """基于 Blackboard、任务认领和 artifact 的有限协作 Runtime。"""
    ...
    def run(
        ...
        final_board = EventDrivenCoordinator(
    ...
    def _to_result(self, board: CollaborationBlackboard, user: UserAccount) -> AgentRunResult:
        ...
        if not response_messages:
            response_messages = self._fallback_messages(
                intent,
                risk,
                user.display_name,
                board.model_input,
    ...
    def _events_to_steps(self, board: CollaborationBlackboard) -> list[AgentStep]:
        ...
        for index, event in enumerate(board.events, start=1):
            ...
            if event.artifact_id:
                detail = (
                    f"{detail}; artifact={event.artifact_id}"
                    if detail
                    else f"artifact={event.artifact_id}"
                )
