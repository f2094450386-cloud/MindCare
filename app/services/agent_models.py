...
class AgentModelRegistry:
    ...
    def profile_for(self, agent_name: str) -> AgentModelProfile:
        ...
        provider = str(self._setting(f"agent_model_{alias}_provider", self._default_provider())).lower()
        model = str(self._setting(f"agent_model_{alias}_model", self._default_model(provider)))
        temperature = float(
            self._setting(f"agent_model_{alias}_temperature", getattr(self.settings, "ai_temperature", 0.35))
        )
        max_tokens = int(
            self._setting(f"agent_model_{alias}_max_tokens", getattr(self.settings, "ai_max_tokens", 512))
        )
