...
class DemoAgent:
    ...
    def act(self, task, board):
        kind = {
            AgentCapability.UNDERSTANDING: "intent",
            AgentCapability.SAFETY: "risk",
        }.get(self.capability, "demo")
        payload = {
            "intent": "CHAT",
            "risk": "LOW",
...
class RegistryAndCoordinatorTests(unittest.TestCase):
    def test_registry_sorts_by_claim_confidence_not_list_order(self):
        ...
        task = AgentTask(
            id="task-1",
            title="Understand",
            required_capabilities=frozenset({AgentCapability.UNDERSTANDING.value}),
        )

        candidates = registry.candidates_for(task, CollaborationBlackboard(turn_id="t1"))

        self.assertEqual(
            [agent.profile.name for agent in candidates],
            ["HighConfidenceAgent", "LowConfidenceAgent"],
        )
    ...
    def test_coordinator_uses_claims_for_open_tasks(self):
        ...
        claimed_tasks = [task for task in result.tasks.values() if task.claimed_by]
        self.assertEqual(
            {name for task in claimed_tasks for name in task.claimed_by},
            {"AgentA", "AgentB"},
        )
