...
class MemoryCompactionTests(unittest.TestCase):
    def test_compaction_keeps_recent_messages_and_adds_internal_summary(self):
        history = [
            AiMessage(role="user" if i % 2 == 0 else "assistant", content=f"第{i}条消息 13800138000")
            for i in range(10)
        ]
        ...
        self.assertEqual(brief, "无可复用的长期事实。")
        self.assertIn("[已脱敏]", compacted[-1].content)
    ...
    def test_compaction_can_be_disabled(self):
        ...
        compacted, brief = compact_history_for_prompt(
            history,
            settings(memory_compaction_enabled=False),
            "",
        )

        self.assertEqual(compacted, history)
        self.assertIn("我最近压力很大", brief)
